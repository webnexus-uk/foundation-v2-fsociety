...
const Pool = function (config, configMain, callback) {
  ...
  this.checkAuxiliary = function (shareData, auxShareData) {
    if (_this.auxiliary.enabled) {
      ...
      const shareDiff =
        Algorithms.ghostrider.diff / Number(_this.auxiliary.rpcData.target);
        ...
        _this.checkAccepted(
          ...
          (accepted, transaction) => {
            ...
            broadcast({
              type: "transaction",
              data: {
                coin: _this.config.primary.coin.name,
  ...
  this.setupSettings = function (callback) {
    ...
    _this.primary.daemon.sendCommands(commands, true, (result) => {
      ...
      _this.statistics.difficulty =
        difficulty * Algorithms.ghostrider.multiplier;
  ...
  this.setupClients = function (client) {
    ...
    client.on("client.socket.error", (error) => {
      _this.emitLog(
        "warning",
        false,
        _this.text.stratumClientText5(client.sendLabel(), JSON.stringify(error))
      );
    });
    client.on("client.socket.timeout", (error) => {
      _this.emitLog(
        "warning",
        false,
        _this.text.stratumClientText6(client.sendLabel(), JSON.stringify(error))
      );
    });
