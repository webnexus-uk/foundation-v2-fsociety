      ...
      JSON.stringify([
        {
          ...
          result: {
            isvalid: true,
            address: "RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1",
          },
        },
...
function mockSocket() {
  ...
  (socket.remoteAddress = "127.0.0.1"), (socket.destroy = () => {});
  socket.setEncoding = () => {};
  socket.setKeepAlive = () => {};
  socket.write = (data) => {
...
describe("Test pool functionality", () => {
  ...
  beforeEach(() => {
    ...
    blockchainDataCopy = JSON.parse(
      JSON.stringify(testdata.getBlockchainInfo())
    );
  ...
  test("Test pool settings setup [2]", (done) => {
    ...
    mockSetupDaemons(pool, () => {
      ...
      pool.setupSettings(() => {
        expect(configCopy.primary.address).toBe(
          "RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1"
        );
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        expect(text).toContain(
          "Could not start pool, error with RPC command response: validateaddress"
        );
        done();
      }
    });
  ...
  test("Test pool settings setup [4]", (done) => {
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        expect(type).toBe("error");
        expect(text).toBe(
          "The daemon reports that the given address is not valid"
        );
        done();
      }
    });
        ...
        .reply(
          ...
          JSON.stringify([
            ...
            { id: "nocktest", error: null, result: { networkhashps: 0 } },
            {
              ...
              result: {
                chain: "main",
                difficulty: { "proof-of-work": 8, "proof-of-stake": 10 },
              },
            },
            {
              id: "nocktest",
  ...
  test("Test pool recipient setup [2]", (done) => {
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        expect(text).toBe(
          "No recipients have been added, which means that no fees will be taken"
        );
        done();
      }
    });
  ...
  test("Test pool manager setup [6]", (done) => {
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        expect(type).toBe("error");
        expect(text).toBe(
          "RPC error with primary daemon instance (127.0.0.1) when submitting block: true"
        );
        done();
      }
    });
  ...
  test("Test pool manager setup [7]", (done) => {
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        expect(type).toBe("error");
        expect(text).toBe(
          "Primary daemon instance (127.0.0.1) rejected a supposedly valid block"
        );
        done();
      }
    });
      ...
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[0][1]).toBe(
            "Submitted a primary block (Fsociety:1) successfully to Fsociety's daemon instance(s)"
          );
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[0][1]).toBe(
            "RPC error with primary daemon instance (127.0.0.1) when requesting a primary template update: true"
          );
          expect(response[1][0]).toBe("error");
          expect(response[1][1]).toBe(
            "RPC error with primary daemon instance when creating the first job"
          );
          done();
        }
      }
  ...
  test("Test pool block polling setup [1]", (done) => {
    ...
    rpcDataCopy.previousblockhash =
      "1d5af7e2ad9aeccb110401761938c07a5895d85711c9c5646661a10407c82769";
    const response = [];
    const pool = new Pool(configCopy, configMainCopy, () => {});
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][1]).toBe(
            "Requested template from primary chain (Fsociety:2) via RPC polling"
          );
          done();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 3) {
          ...
          expect(response[1][0]).toBe("log");
          expect(response[1][1]).toBe(
            "Requested template from auxiliary chain (Namecoin:2) via RPC polling"
          );
          expect(response[2][0]).toBe("log");
          expect(response[2][1]).toBe(
            "Requested template from primary chain (Fsociety:1) via RPC polling"
          );
          done();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("error");
          expect(response[1][1]).toBe(
            "RPC error with auxiliary daemon instance (127.0.0.1) when requesting an auxiliary template update: true"
          );
          done();
        }
      }
          ...
          mockSetupAuxiliaryBlockchain(pool, () => {
            mockSetupFirstJob(pool, () => {
              pool.setupNetwork(() => {
                ...
                expect(typeof pool.network.broadcastMiningJobs).toBe(
                  "function"
                );
  ...
  test("Test pool stratum setup [2]", (done) => {
    ...
    pool.on("pool.log", (type, text) => {
      ...
      if (response.length === 9) {
        ...
        expect(response[8][1]).toBe(
          "No new blocks for 60 seconds. Updating transactions and rebroadcasting work"
        );
        pool.network.stopNetwork();
      }
    });
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("log");
          expect(response[1][1]).toBe(
            "Difficulty update queued for worker: RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1 (8)"
          );
          pool.network.stopNetwork();
        }
      }
  ...
  test("Test pool stratum setup [7]", (done) => {
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("log");
          expect(response[1][1]).toBe(
            "Difficulty updated successfully for worker: RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1 (8)"
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("client.socket.success", () => {
      pool.difficulty[client.socket.localPort] = {
        clients: { test: ["test"] },
      };
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][1]).toBe(
            "A client (client [example]) sent a malformed message to the server: test"
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("warning");
          expect(response[1][1]).toBe(
            "Socket flooding was detected from a client (client [example])"
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("warning");
          expect(response[1][1]).toBe(
            'A socket error was detected from a client (client [example]): "test"'
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("warning");
          expect(response[1][1]).toBe(
            'A client (client [example]) was timed out from the server: "test"'
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("warning");
          expect(response[1][1]).toBe(
            "A client (client [example]) disconnected from the server"
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("warning");
          expect(response[1][1]).toBe(
            "Rejected incoming connection (client [example]). The client is banned for 1000 seconds"
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("log");
          expect(response[1][1]).toBe(
            "Forgave banned client (client [example]). They can now reconnect to the pool"
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("warning");
          expect(response[1][1]).toBe(
            "Because of malicious behavior, a client (client [example]) has been banned"
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("warning");
          expect(response[1][1]).toBe(
            "A client (client [example]) sent an unknown stratum method to the server: test"
          );
          pool.network.stopNetwork();
        }
      }
  ...
  test("Test pool stratum setup [20]", (done) => {
    ...
    pool.on("client.socket.success", () => {
      ...
      client.emit(
        "client.submit",
        { params: [null, "id", "extraNonce", "time", "nonce", "version"] },
        (error, result) => {
          pool.network.on("network.stopped", () => done());
  ...
  test("Test pool stratum authentication [2]", (done) => {
    ...
    mockSetupDaemons(pool, () => {
      ...
      pool.checkWorker(
        pool.primary.daemon,
        "RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1.worker1",
        (valid) => {
          expect(valid).toBe(true);
          done();
        }
  ...
  test("Test pool workers handling [1]", () => {
    ...
    const blocks1 = {
      ...
      solo: false,
      transaction:
        "01905e07123f9edb0439376639ad530207f9e313faee4708418a45bfc43e52be",
      type: "primary",
      previous: "pending",
    };
    const worker1 = {
    ...
    const expected = {
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1: { immature: 10000, generate: 0 },
    };
  ...
  test("Test pool workers handling [2]", () => {
    ...
    const expected = {
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1: { generate: 0, immature: 20000 },
    };
    pool.handlePrimaryWorkers(
      [blocks1, blocks1],
      [[worker1], [worker1]],
      false,
      (results) => {
  ...
  test("Test pool workers handling [4]", () => {
    ...
    const expected = {
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1: {
        immature: 8474.10994411,
        generate: 0,
      },
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G2: {
        immature: 1525.89005589,
        generate: 0,
      },
    };
    pool.handlePrimaryWorkers(
  ...
  test("Test pool workers handling [5]", () => {
    ...
    const expected = {
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1: { generate: 10000, immature: 0 },
    };
    pool.handlePrimaryWorkers([blocks1], [[worker1]], false, (results) => {
      expect(results).toStrictEqual(expected);
    });
  ...
  test("Test pool workers handling [6]", () => {
    ...
    const expected = {
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1: { immature: 0, generate: 20000 },
    };
    pool.handlePrimaryWorkers(
      [blocks1, blocks1],
      [[worker1], [worker1]],
  ...
  test("Test pool workers handling [9]", () => {
    ...
    const expected = {
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1: { immature: 0, generate: -10000 },
    };
    pool.handlePrimaryWorkers([blocks1], [[worker1]], false, (results) => {
      expect(results).toStrictEqual(expected);
    });
  ...
  test("Test pool workers handling [12]", () => {
    ...
    const expected = {
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1: {
        immature: 9570.0056964,
        generate: 0,
      },
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G2: {
        immature: 429.9943036,
        generate: 0,
      },
    };
    pool.handlePrimaryWorkers(
  ...
  test("Test pool workers handling [14]", () => {
    ...
    const expected = {
      RHP3VKiSYH4putQeSKLwr47QDdERa6H6G1: { immature: -10000, generate: 10000 },
    };
    pool.handlePrimaryWorkers([blocks1], [[worker1]], true, (results) => {
      expect(results).toStrictEqual(expected);
    });
  ...
  test("Test pool payments handling [2]", (done) => {
    ...
    mockSetupDaemons(pool, () => {
      pool.handlePrimaryPayments(
        payments,
        (error, amounts, balances, transaction) => {
          expect(error).toBe(null);
  ...
  test("Test pool payments handling [8]", (done) => {
    const auxiliaryPaymentsConfigCopy = Object.assign(
      {},
      auxiliaryPaymentsConfig
    );
